...
public:
  ...
  void processLoop();
  void setup();
  int  set_cmd_once(const std::string &cmd, int length, const std::string &expected_response);
  int  set_cmd(const std::string &cmd, int length, const std::string &expected_response);
  bool waitForResponse(const std::string &expected_response, int timeout_ms);
  bool waitForResponse1(int timeout_ms);
