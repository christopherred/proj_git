...
#include <thread>  // 包含std::this_thread::sleep_for
#include <chrono>  // 包含std::chrono::seconds
#define  ttime  1000 //接收串口消息和发送串口消息时间间隔（ms）
#define  ttime1  2500 //imu重启等待时间
double initial_heading = 0.0;
bool initial_heading_set = false;
...
: rclcpp::Node ("ahrs_bringup")
{
  ...
  // Add: 上电自动调平
  RCLCPP_INFO(this->get_logger(),"Begin leveling");
  ...
  if (serial_.isOpen())
  {
    ...
    wait_time(ttime);
    // 进入配置模式
    std::string command1 = "#fconfig\r\n";
    set_cmd(command1, 12, "OK");
    // const uint8_t* c_command1 = reinterpret_cast<const uint8_t*>(command1.c_str());
    // serial_.write(c_command1, 12);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout1");
    // }

    // 发送调平命令
    wait_time(ttime);
    std::string command2 = "#fimucal_level\r\n";
    set_cmd(command2, 18, "OK");
    // const uint8_t* c_command2 = reinterpret_cast<const uint8_t*>(command2.c_str());
    // serial_.write(c_command2, 18);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout2");
    // }
    //保存
    wait_time(ttime);
    std::string command3 = "#fsave\r\n";
    set_cmd(command3, 10, "OK");
    // const uint8_t* c_command3 = reinterpret_cast<const uint8_t*>(command3.c_str());
    // serial_.write(c_command3, 10);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout3");
    // }
    // //重启
    // wait_time(ttime);
    // std::string command4 = "#freboot\r\n";
    // set_cmd(command4, 12);
    // const uint8_t* c_command4 = reinterpret_cast<const uint8_t*>(command4.c_str());
    // serial_.write(c_command4, 12);
    // if (!waitForResponse("y/n", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout4");
    // }
    //重启确认
    // wait_time(ttime);
    // std::string command5 = "y\r\n";
    // set_cmd(command5, 5);
    // const uint8_t* c_command5 = reinterpret_cast<const uint8_t*>(command5.c_str());
    // serial_.write(c_command5, 5);
    //   if (!waitForResponse1(8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout5");
    // }

    // wait_time(ttime1);
    // std::string command6 = "#fconfig\r\n";
    // set_cmd(command6, 12);
    // const uint8_t* c_command6 = reinterpret_cast<const uint8_t*>(command6.c_str());
    // serial_.write(c_command6, 12);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout6");
    // }
    // // 发送调平命令
    wait_time(ttime);
    std::string command7 = "#fimucal_acce\r\n";
    set_cmd(command7, 17, "OK");
    // const uint8_t* c_command7 = reinterpret_cast<const uint8_t*>(command7.c_str());
    // serial_.write(c_command7, 17);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout7");
    // }
    //保存
    wait_time(ttime);
    std::string command8 = "#fsave\r\n";
    set_cmd(command8, 10, "OK");
    // const uint8_t* c_command8 = reinterpret_cast<const uint8_t*>(command8.c_str());
    // serial_.write(c_command8, 10);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout8");
    // }
    // //重启
    // wait_time(ttime);
    // std::string command9 = "#freboot\r\n";
    // set_cmd(command9, 12);
    // const uint8_t* c_command9 = reinterpret_cast<const uint8_t*>(command9.c_str());
    // serial_.write(c_command9, 12);
    // if (!waitForResponse("y/n", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout9");
    // }
    //重启确认
    // wait_time(ttime);
    // std::string command10 = "y\r\n";
    // set_cmd(command10, 4);
    // const uint8_t* c_command10 = reinterpret_cast<const uint8_t*>(command10.c_str());
    // serial_.write(c_command10, 5);
    // if (!waitForResponse1(8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout10");
    // }

    // wait_time(ttime1);
    // std::string command11 = "#fconfig\r\n";
    // set_cmd(command11, 12);
    // const uint8_t* c_command11 = reinterpret_cast<const uint8_t*>(command11.c_str());
    // serial_.write(c_command11, 12);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout11");
    // }
    // 发送调平命令
    wait_time(ttime);
    std::string command12 = "#fimucal_gyro\r\n";
    set_cmd(command12, 17, "OK");
    // const uint8_t* c_command12 = reinterpret_cast<const uint8_t*>(command12.c_str());
    // serial_.write(c_command12, 17);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout12");
    // }
    //保存
    wait_time(ttime);
    std::string command13 = "#fsave\r\n";
    set_cmd(command13, 10, "OK");
    // const uint8_t* c_command13 = reinterpret_cast<const uint8_t*>(command13.c_str());
    // serial_.write(c_command13, 10);
    // if (!waitForResponse("OK", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout13");
    // }
    //重启
    wait_time(ttime);
    std::string command14 = "#freboot\r\n";
    set_cmd(command14, 12, "y/n");
    // const uint8_t* c_command14 = reinterpret_cast<const uint8_t*>(command14.c_str());
    // serial_.write(c_command14, 12);
    // if (!waitForResponse("y/n", 8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout14");
    // }
    //重启确认
    wait_time(ttime);
    std::string command15 = "y\r\n";
    set_cmd(command15, 5, "");
    // const uint8_t* c_command15 = reinterpret_cast<const uint8_t*>(command15.c_str());
    // serial_.write(c_command15, 5);
    // if (!waitForResponse1(8000)) {
    //   RCLCPP_ERROR(this->get_logger(), "Timeout15");
    // }
    RCLCPP_INFO(this->get_logger(), "End leveling success!");
  }
...
int ahrsBringup::set_cmd_once(const std::string &cmd, int length, const std::string &expected_response){
  const uint8_t* c_command1 = reinterpret_cast<const uint8_t*>(cmd.c_str());
  serial_.write(c_command1, length);
  if (!waitForResponse(expected_response, 2000)) {
    // 发生超时
    return 1;
  }else{
    // 正常相应
...
int ahrsBringup::set_cmd(const std::string &cmd, int length, const std::string &expected_response){
  int error_time = 0;
  while ( error_time < 10 ){
    if ( set_cmd_once(cmd, length, expected_response) == 1 ){
      // 发生超时
      RCLCPP_WARN(this->get_logger(), "IMU Timeout Once in Command [ %s ]", cmd.substr(0, cmd.length()-2).c_str());
      error_time += 1;
    }else{
      // 正常相应
      return 0;
  ...
  RCLCPP_ERROR(this->get_logger(), "IMU Timeout in Command [ %s ]", cmd.substr(0, cmd.length()-2).c_str());
  return 1;
}
...
bool ahrsBringup::waitForResponse(const std::string &expected_response, int timeout_ms) {
    std::string response;
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(timeout_ms)) {
        if (serial_.available()) {
            ...
            if (response.find(expected_response) != std::string::npos) {
                RCLCPP_INFO(this->get_logger(), "Received response: %s", response.substr(0, response.length()-2).c_str()); // 添加调试信息
                return true;  // 找到期望的响应
            }
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(10));  // 短暂休眠，避免占用过多CPU资源
    }
    return false;  // 超时
}
bool ahrsBringup::waitForResponse1(int timeout_ms) {
    std::string response;
    auto start = std::chrono::steady_clock::now();

    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(timeout_ms)) {
        if (serial_.available()) {
            //serial_.read(serial_.available());  // 读取所有可用的数据
            response += serial_.read(serial_.available());
            RCLCPP_INFO(this->get_logger(), "Received response: %s", response.c_str()); // 添加调试信息
            return true;  // 只要有数据传输过来就返回 true
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(10));  // 短暂休眠，避免占用过多CPU资源
    }
    return false;  // 超时
...
bool ahrsBringup::wait_time( int timeout_ms) {
    auto start = std::chrono::steady_clock::now();
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(timeout_ms)) {
       continue;
    }
...
void ahrsBringup::processLoop()  // 数据处理过程
{
  ...
  while (rclcpp::ok())
  {
    ...
    else if (head_type[0] == TYPE_AHRS)
    {
      ...
      double current_heading = ahrs_frame_.frame.data.data_pack.Heading;

      // if (!initial_heading_set)
      // {
      //   initial_heading = current_heading;  // 捕获初始偏航角
      //   initial_heading_set = true;
      // }

      // pose_2d.theta = ahrs_frame_.frame.data.data_pack.Heading;
      pose_2d.theta = current_heading - initial_heading;
