...
class Robot():    
    def __init__(self, name, clientID):
        ...
        self.robot           = sim.simxGetObjectHandle(clientID, 'robot',  sim.simx_opmode_blocking)[1]
        self.LM_motor        = sim.simxGetObjectHandle(clientID, 'LM_motor', sim.simx_opmode_blocking)[1]
        self.RM_motor        = sim.simxGetObjectHandle(clientID, 'RM_motor', sim.simx_opmode_blocking)[1]    
        # 打印获取的关节句柄，确认初始化成功
        print(f'Robot: {self.robot}, LM_motor: {self.LM_motor}, RM_motor: {self.RM_motor}')
    
class ROSRobot(Robot, Node):    
    def __init__(self, name, clientID):
        # 初始化 Node 父类
        Node.__init__(self, 'simulation_diff_base_driver')
    ...
    def pub_wheel_data(self):
        # 获取各轮子的速度
        result, joint1_vel = sim.simxGetObjectFloatParameter(self.clientID, self.LM_motor, 2012, sim.simx_opmode_blocking)
        result, joint2_vel = sim.simxGetObjectFloatParameter(self.clientID, self.RM_motor, 2012, sim.simx_opmode_blocking)

        # 获取转矩
        result, torque1 = sim.simxGetJointForce(self.clientID, self.LM_motor, sim.simx_opmode_blocking)
        result, torque2 = sim.simxGetJointForce(self.clientID, self.RM_motor, sim.simx_opmode_blocking)
        
        # 发布轮子速度
        wheel_speed_msg = Float32MultiArray()
        wheel_speed_msg.data = [joint1_vel, joint2_vel]  # 各轮子的速度
        self.pub_wheelspeed.publish(wheel_speed_msg)

        # 发布轮子扭矩
        wheel_torque_msg = Float32MultiArray()
        wheel_torque_msg.data = [torque1, torque2]  # 各轮子的扭矩
    ...
    def update_wheel_speed(self, msg: Float32MultiArray):
        ...
        sim.simxSetJointTargetVelocity(self.clientID, self.LM_motor, msg.data[0]/10, sim.simx_opmode_streaming)
        sim.simxSetJointTargetVelocity(self.clientID, self.RM_motor, -msg.data[1]/10, sim.simx_opmode_streaming) 
...
def main():
    ...
    if clientID != -1:
        ...
        newrobot = ROSRobot('robot', clientID)  # 创建RosRobot类实例
        print("init.")
        try:
