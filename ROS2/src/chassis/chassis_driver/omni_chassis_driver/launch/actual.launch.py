...
def generate_launch_description():
    ...
    # 发布 radar 和 base_link 相对位置的静态 TF
    static_transform_radar = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='static_transform_radar',
        output='screen',
        arguments=['--x', '0',
                  '--y', '0.13167',
                  '--z', '0',
                  ...
                  '--qw', '1',
                  '--frame-id', 'base_link',
                  '--child-frame-id', 'radar']
    )

    # 发布 camera 和 base_link 相对位置的静态 TF
    ...
    declare_use_global_controller = DeclareLaunchArgument(
        ...
        default_value='false',     # 默认值，可以设置为 'false'
    ...
    declare_use_simulation = DeclareLaunchArgument(
        'use_simulation',  # 参数名称
        default_value='false',  # 默认值，可以设置为 'true'
    ...
    base_controller_global_node = Node(
        ...
        executable='omni_base_controller_global_actual_node',
        name='omni_base_controller_global_actual_node',
        output='screen',
        emulate_tty=True,
    ...
    base_controller_local_node = Node(
        package='omni_chassis_driver',
        executable='omni_base_controller_local_actual_node',
        name='omni_base_controller_local_actual_node',
        output='screen',
        emulate_tty=True,
        parameters=[{'wheel_radius': 0.0375}, {'wheel_spacing': 0.175}],
